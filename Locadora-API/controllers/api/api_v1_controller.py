...
@api_v1_bp.route("/locacoes", methods=["GET"]) #* TODO Qual é a rota?
def api_listar_locacoes():
    locacoes = Locacao.listar_com_detalhes()
    return jsonify([
        {
           #* TODO Continue o metodo
            "id": loc.id,
...
def api_detalhe_veiculo(veiculo_id):
    ...
    if not veiculo:
        return jsonify({"erro": "Veículo não encontrado"}), #* TODO QUAL ERRO APARECE QUANDO NÃO ENCONTRA O VEÍCULO?? "Veículo não encontrado"
    return jsonify({
        "id": veiculo.id,
        "placa": veiculo.placa,
...
def api_modelos_nhtsa():
    marca = request.args.get("marca", "Honda") #* TODO responda por que estamos utilizando o request.args.get? Pegar o parâmetro marca o honda ali seria o padrão
    try:
